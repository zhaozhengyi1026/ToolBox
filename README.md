...
- 拆分、转图片、压缩各有独立入口，每个工具都支持同时处理多份 PDF；
- 把 PDF 按单页拆分，或自由组合指定页码；
- 在网页上修改每份拆分文件的名称，可单独下载或打包下载；
- 支持输入页码、单击或按住鼠标划过缩略图选页，每 15 页翻一页；
- 把选中的页面转换为 PNG / JPG，可选择输出清晰度；
...
- 一次上传多张 JPG、PNG 或 WebP 图片；
- 自由选择图片组成多份 PDF，每份可重命名并调整图片顺序；
- 批量压缩图片，显示每张图片压缩前后的体积；
- 多份结果自动打包下载。

浏览器版入口位于 [`image-tools`](image-tools/index.html)。
